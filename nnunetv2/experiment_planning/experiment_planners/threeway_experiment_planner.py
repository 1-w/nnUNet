...
class ThreewayExperimentPlanner(object):
    def __init__(
        ...
        if isfile(join(self.raw_dataset_folder, "splits_final.json")):
            _maybe_copy_splits_file(
                join(self.raw_dataset_folder, "splits_final.json"),
                join(preprocessed_folder, "splits_final.json"),
            )

    def determine_reader_writer(self):
        example_image = self.dataset[self.dataset.keys().__iter__().__next__()][
            "images"
        ][0]
        return determine_reader_writer_from_dataset_json(
            self.dataset_json, example_image
        )
    ...
    def save_plans(self, plans):
        ...
        print(
            f"Plans were saved to {join(nnUNet_preprocessed, self.dataset_name, self.plans_identifier + '.json')}"
        )
