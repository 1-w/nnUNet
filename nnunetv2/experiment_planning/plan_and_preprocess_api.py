...
def plan_experiment_dataset(
    ...
    planner = experiment_planner_class(
        dataset_id,
    ...
    for k in dataset:
        copy_file(
            ...
            join(
                ...
                dataset_name,
                "gt_segmentations",
                k + dataset_json["file_ending"],
            ),
            update=True,
        )


def preprocess_threeway_dataset(
    dataset_id: int,
    plans_identifier: str = "nnUNetPlans",
    configurations: Union[Tuple[str], List[str]] = ("2d", "3d_fullres", "3d_lowres"),
    num_processes: Union[int, Tuple[int, ...], List[int]] = (8, 4, 8),
    ...
    dataset_name = convert_id_to_dataset_name(dataset_id)
    print(f"Preprocessing threeway dataset {dataset_name}")
    plans_file = join(nnUNet_preprocessed, dataset_name, plans_identifier + ".json")
    plans_manager = PlansManager(plans_file)
    for n, c in zip(num_processes, configurations):
        print(f"Configuration: {c}...")
    ...
    maybe_mkdir_p(join(nnUNet_preprocessed, dataset_name, "gt_segmentations"))
    maybe_mkdir_p(join(nnUNet_preprocessed, dataset_name, "other_segmentations"))
    dataset_json = load_json(join(nnUNet_raw, dataset_name, "dataset.json"))
...
def preprocess(
    ...
    pp_func = preprocess_dataset
    if threeway:
        pp_func = preprocess_threeway_dataset

    for d in dataset_ids:
        pp_func(d, plans_identifier, configurations, num_processes, verbose)
