...
def get_filenames_of_train_images_and_targets_and_othertargets(
    raw_dataset_folder: str, dataset_json: dict = None
):
    if dataset_json is None:
        dataset_json = load_json(join(raw_dataset_folder, "dataset.json"))
    ...
    if "dataset" in dataset_json.keys():
        ...
        for k in dataset.keys():
            ...
            dataset[k]["otherlabel"] = (
                os.path.abspath(join(raw_dataset_folder, dataset[k]["otherlabel"]))
                if not os.path.isabs(dataset[k]["otherlabel"])
                else dataset[k]["otherlabel"]
            )
            dataset[k]["images"] = [
                (
                    os.path.abspath(join(raw_dataset_folder, i))
        ...
        segs2 = [
            join(raw_dataset_folder, "otherlabelsTr", i + dataset_json["file_ending"])
            for i in identifiers
        ]
        dataset = {
            i: {"images": im, "label": se, "otherlabel": se2}
            for i, im, se, se2 in zip(identifiers, images, segs, segs2)
        }
    return dataset
