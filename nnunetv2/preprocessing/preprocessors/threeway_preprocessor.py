...
class ThreewayPreprocessor(object):
    ...
    def run_case_npy(
        ...
        seg2: Union[np.ndarray, None] = None,
    ):
        ...
        if seg2 is not None:
            assert (
                data.shape[1:] == seg2.shape[1:]
            ), "Shape mismatch between image and segmentation. Please fix your dataset and make use of the --verify_dataset_integrity flag to ensure everything is correct"
            seg2 = np.copy(seg2)

        has_seg = seg is not None
        has_seg2 = seg2 is not None
        ...
        if seg2 is not None:
            seg2 = seg2.transpose(
                [0, *[i + 1 for i in plans_manager.transpose_forward]]
            )
        original_spacing = [
        ...
        _, seg2, _ = crop_to_nonzero(olddata, seg2)
        ...
        seg2 = configuration_manager.resampling_fn_seg(
            seg2, new_shape, original_spacing, target_spacing
        )
        if self.verbose:
            print(
        ...
        if has_seg:
            ...
            seg2 = self.modify_seg_fn(
                seg2, plans_manager, dataset_json, configuration_manager
            )
        if np.max(seg) > 127:
        ...
        if np.max(seg2) > 127:
            ...
            seg2 = seg2.astype(np.int8)
        return data, seg, seg2
    ...
    def run_case(
        ...
        if seg2_file is not None:
            seg2, _ = rw.read_seg(seg2_file)
        else:
            seg2 = None

        data, seg, seg2 = self.run_case_npy(
            data,
            seg,
            data_properties,
            plans_manager,
    ...
    def run_case_save(
        ...
        np.savez_compressed(
            output_filename_truncated + ".npz", data=data, seg=seg, seg2=seg2
        )
    ...
    def _normalize(
        ...
        for c in range(data.shape[0]):
            ...
            if normalizer_class is None:
                raise RuntimeError(
                    f"Unable to locate class '{scheme}' for normalization"
                )
...
def example_test_case_preprocessing():
    ...
    data, _, properties, seg2 = pp.run_case(
        input_images,
        seg_file=None,
