...
class nnUNetThreewayDataset(nnUNetDataset):
    ...
    def __init__(
        ...
        for c in case_identifiers:
            if folder_with_segs_from_previous_stage is not None:
                ...
                self.dataset[c]["seg2_from_prev_stage_file"] = join(
                    folder_with_segs_from_previous_stage, f"{c}.npz"
                )  # TODO: This is a placeholder. This should be changed to the correct file name
    ...
    def load_case(self, key):
        ...
        if "open_seg2_file" in entry.keys():
            seg2 = entry["open_seg2_file"]
            # print('using open data file')
        elif isfile(
            entry["data_file"][:-4] + "_seg2.npy"
        ):  # TODO: This is a placeholder. This should be changed to the correct file name
            seg2 = np.load(
                entry["data_file"][:-4] + "_seg2.npy", "r"
            )  # TODO: This is a placeholder. This should be changed to the correct file name
            if self.keep_files_open:
                self.dataset[key]["open_seg2_file"] = seg2
                # print('saving open seg file')
        else:
            seg2 = np.load(entry["data_file"])["seg2"]
        ...
        if "seg2_from_prev_stage_file" in entry.keys():
            if isfile(entry["seg2_from_prev_stage_file"][:-4] + ".npy"):
                seg2_prev = np.load(
                    entry["seg2_from_prev_stage_file"][:-4] + ".npy", "r"
                )
            else:
                seg2_prev = np.load(entry["seg2_from_prev_stage_file"])["seg2"]
            seg2 = np.vstack((seg2, seg2_prev[None]))

        return data, seg, seg2, entry["properties"]
