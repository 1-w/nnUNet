    ...
    try:
        ...
        if overwrite_existing or not isfile(data_npy):
            try:
                ...
                print(
                    f"Unable to open preprocessed file {npz_file}. Rerun nnUNetv2_preprocess!"
                )
                raise e
        ...
        if verify_npy:
            try:
                ...
                # os.remove(seg2_npy)
                print(
                    # f"Error when checking {data_npy} and {seg_npy} and {seg2_npy}, fixing..."
                    f"Error when checking {data_npy} and {seg_npy}, fixing..."
                )
                if fail_ctr < 2:
                    _convert_to_npy(
                        npz_file,
