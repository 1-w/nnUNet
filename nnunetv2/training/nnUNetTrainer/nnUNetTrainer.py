...
class nnUNetTrainer(object):
    ...
    def _do_i_compile(self):
        ...
        if self.device == torch.device("mps"):
            if "nnUNet_compile" in os.environ.keys() and os.environ[
                "nnUNet_compile"
            ].lower() in ("true", "1", "t"):
                self.print_to_log_file(
                    "INFO: torch.compile disabled because of unsupported mps device"
                )
            return False
        ...
        if self.device == torch.device("cpu"):
            if "nnUNet_compile" in os.environ.keys() and os.environ[
                ...
                self.print_to_log_file(
                    "INFO: torch.compile disabled because device is CPU"
                )
            return False
        ...
        if os.name == "nt":
            if "nnUNet_compile" in os.environ.keys() and os.environ[
                ...
                self.print_to_log_file(
                    "INFO: torch.compile disabled because Windows is not natively supported. If "
        ...
        if not self.is_ddp:
            ...
            oversample = [
                (
                    ...
                    if not i
                    < round(
                        global_batch_size * (1 - self.oversample_foreground_percent)
                    )
                    else False
                )
            ...
            if sample_id_high / global_batch_size < (
                ...
                oversample_percent = (
                    sum(oversample[sample_id_low:sample_id_high])
...
class nnUNetTrainer5000(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
        ...
        super(nnUNetTrainer5000, self).__init__(
...
class nnUNetTrainer5000GradAcc(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
        ...
        super(nnUNetTrainer5000GradAcc, self).__init__(
            plans, configuration, fold, dataset_json, unpack_dataset, device
        )
        self.num_epochs = 2000
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 100
        self.num_iterations_gradient_accumulation = 4
    ...
    def train_step(self, batch: dict, step=True) -> dict:
        data = batch["data"]
        target = batch["target"]
        ...
        with (
            ...
            # del data
            l = self.loss(output, target)
            l = l / self.num_iterations_gradient_accumulation
        ...
        return {
            "loss": l.detach().cpu().numpy() * self.num_iterations_gradient_accumulation
        }
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                train_outputs.append(
                    self.train_step(
                        ...
                        step=(batch_id + 1) % self.num_iterations_gradient_accumulation
                        == 0,
                    )
                )
            self.on_train_epoch_end(train_outputs)
...
class HeteroModalnnUNetTrainer(nnUNetTrainer):
    def __init__(
        ...
        super(HeteroModalnnUNetTrainer, self).__init__(
            plans, configuration, fold, dataset_json, unpack_dataset, device
        )
        self.num_epochs = 2200
        self.num_iterations_per_epoch = 250
        self.rnd = np.random.RandomState(69)

        # this is to simulate missing data is num_channels
        # self.comparison = torch.Tensor(
        #     [[1]*self.batch_size
        #     ]* len(
        #         self.plans_manager.plans["foreground_intensity_properties_per_channel"]
        #     )
        # )
        # self.comparison = self.comparison.to(self.device)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # randomly set comparison to 0 making it increasingly likely with training time
        ...
        for b in range(comp.shape[0]):  # batch dim
            for i in indices[:-1]:
                # check if data is left, dont wanna make too much missing
                if comp[b].sum() == 1:
                    break
                if np.random.random() < self.current_epoch / self.num_epochs:
                    # if np.random.random() < nnunet_trainer.current_epoch / nnunet_trainer.num_epochs:
                    comp[b, i] = 0
                    data[b, i] = 0  # simulate data as missing
        ...
        return {
            "loss": l.detach().cpu().numpy(),
            "comparison": comp.detach().cpu().numpy(),
        }
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            print("IS DDP")
            losses_tr = [None for _ in range(dist.get_world_size())]
            comps_tr = [None for _ in range(dist.get_world_size())]

            dist.all_gather_object(losses_tr, outputs["loss"])
            dist.all_gather_object(comps_tr, outputs["comparison"])

            loss_here = np.vstack(losses_tr).mean()
            comps_here = np.vstack(comps_tr).mean(axis=0)
        else:
            loss_here = np.mean(outputs["loss"])
            comps_here = np.mean(
                outputs["comparison"].reshape(
                    outputs["comparison"].shape[0] * outputs["comparison"].shape[1], 3
                ),
                axis=0,
            )

        self.logger.log("missing_data", comps_here, self.current_epoch)
        self.logger.log("train_losses", loss_here, self.current_epoch)
        self.logger.log(
            "mod_weights",
            self.network.encoder.heteromodel_block.mod_weights.detach().cpu().numpy(),
            self.current_epoch,
        )
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file(
            "missing_data",
            np.round(self.logger.my_fantastic_logging["missing_data"][-1], decimals=4),
        )

        self.print_to_log_file(
            "mod_weights",
            np.round(self.logger.my_fantastic_logging["mod_weights"][-1], decimals=4),
        )
...
class HeteroModalnnUNetTrainerAdam(HeteroModalnnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
...
class combinedTrainer(nnUNetTrainer):
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.label_manager.has_regions:
            ...
            predicted_segmentation_onehot.scatter_(1, output_seg, 1)
            del output_seg
            # predicted_segmentation_onehot = 1 * (output > 0.5)
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        for i in range(2):
            train_outputs.append(
                nnunet_trainer.train_step(next(nnunet_trainer.dataloader_train))
            )
        ##%%
    ...
    for val in ["best", "final"]:
        for fold in range(5):

            ckpt_path = f"/home/co-walg1/rds/rds-kw350-meld/meld_fe/nnunet_data/nnUNet_results/Dataset101_MMMPV/HeteroModalnnUNetTrainer__heteromodalPlan__3d_fullres/fold_{fold}/checkpoint_{val}.pth"

            if not os.path.isfile(ckpt_path):
                print(f"Skipping checkpoint for {val} fold {fold}")
                continue

            checkpoint = torch.load(ckpt_path, map_location=torch.device("cpu"))
            torch.save(checkpoint, ckpt_path.replace(".pth", "_wrong_arch.pth"))

            ## %%

            new_checkpoint = copy.deepcopy(checkpoint)

            new_weights = nnunet_trainer.network.state_dict()
            ...
            for k, v in checkpoint["network_weights"].items():
                ...
                if new_weights[k].shape != v.shape:
                    parts = k.split(".")
                    stage = int(parts[2])
                    new_stage = stage + 1
                    new_parts = parts
                    new_parts[2] = str(new_stage)
                    new_k = ".".join(new_parts)
                    assert (
                        checkpoint["network_weights"][new_k].shape
                        == new_weights[k].shape
                    )
                    updated_weights[k] = checkpoint["network_weights"][new_k]
                else:
                    updated_weights[k] = v

            new_checkpoint["network_weights"] = updated_weights

            ## %%
            len(list(nnunet_trainer.network.parameters()))
            ## %%

            current_shapes = [p.shape for p in nnunet_trainer.network.parameters()]
            checkpoint_shapes = [
                p["momentum_buffer"].shape
                for p in checkpoint["optimizer_state"]["state"].values()
            ]

            ## %%
            current_idx = 0
            checkpoint_idx = 0
            corresponding_current_index_and_checkpoint_index = {}

            for i, p in enumerate(checkpoint_shapes):
                if p != current_shapes[current_idx]:
                    continue
                corresponding_current_index_and_checkpoint_index[current_idx] = i
                current_idx += 1

            ## %%
            new_optimizer_state = {
                k: checkpoint["optimizer_state"]["state"][v]
                for k, v in corresponding_current_index_and_checkpoint_index.items()
            }

            new_checkpoint["optimizer_state"]["state"] = new_optimizer_state
            new_checkpoint["optimizer_state"]["param_groups"][0]["params"] = list(
                new_optimizer_state.keys()
            )

            try:
                nnunet_trainer.load_checkpoint(new_checkpoint)
            except:
