...
class nnUNetHeteroTrainer(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
        ...
        # this is to simulate missing data
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        # randomly set comparison to 0 making it increasingly likely with epochs

        # seed=69 + self.optimizer.state[self.optimizer.param_groups[0]["params"][-1]]["step"])
        ...
        data = data.to(self.device, non_blocking=True)
        comp = comp.to(self.device, non_blocking=True)
        if isinstance(target, list):
            target = [i.to(self.device, non_blocking=True) for i in target]
        else:
            target = target.to(self.device, non_blocking=True)
        ...
        return {
            "loss": l.detach().cpu().numpy(),
            "comparison": np.ceil(comp.detach().cpu().numpy()),
        }
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            loss_here = np.mean(outputs["loss"])
            comps_here = np.mean(outputs["comparison"], axis=0)
        self.logger.log("missing_data", comps_here, self.current_epoch)
        self.logger.log("train_losses", loss_here, self.current_epoch)
        self.logger.log(
            "mod_weights",
            self.network.mod_weights.detach().cpu().numpy(),
            self.current_epoch,
        )
...
class nnUNetThreewayTrainer(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if isinstance(target, list):
            target = [i.to(self.device, non_blocking=True) for i in target]
            target2 = [i.to(self.device, non_blocking=True) for i in target2]
        else:
            target = target.to(self.device, non_blocking=True)
            target2 = target2.to(self.device, non_blocking=True)
        ...
        with (
            ...
            output = self.network(data)
            # del data
            l = self.loss(output, target, target2)
    ...
    def _build_loss(self):
        loss = TC_and_CE_loss(
            ...
            weight_dice=1,
            ignore_label=self.label_manager.ignore_label,
            dice_class=MemoryEfficientSoftThriceLoss,
        )
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            dl_tr = nnUNetThreewayDataLoader3D(
                dataset_tr,
                self.batch_size,
                initial_patch_size,
                self.configuration_manager.patch_size,
            ...
            dl_val = nnUNetThreewayDataLoader3D(
                dataset_val,
                self.batch_size,
                self.configuration_manager.patch_size,
                self.configuration_manager.patch_size,
    ...
    def get_tr_and_val_datasets(self):
        ...
        # load the datasets for training and validation. Note that we always draw random samples so we really don't
        # care about distributing training cases across GPUs.
        dataset_tr = nnUNetThreewayDataset(
            self.preprocessed_dataset_folder,
            tr_keys,
            folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
            num_images_properties_loading_threshold=0,
        )
        dataset_val = nnUNetThreewayDataset(
            self.preprocessed_dataset_folder,
            val_keys,
            folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
            num_images_properties_loading_threshold=0,
    ...
    def get_training_transforms(
        ...
        if mirror_axes is not None and len(mirror_axes) > 0:
            tr_transforms.append(MirrorTransform3(mirror_axes))
        ...
        tr_transforms.append(RemoveLabelTransform(-1, 0))
        tr_transforms.append(
            RemoveLabelTransform(-1, 0, input_key="seg2", output_key="seg2")
        )
        ...
        if is_cascaded:
            ...
            tr_transforms.append(
                MoveSegAsOneHotToData(1, foreground_labels, "seg2", "data")
            )
            tr_transforms.append(
                ApplyRandomBinaryOperatorTransform(
                    channel_idx=list(range(-len(foreground_labels), 0)),
        ...
        tr_transforms.append(RenameTransform("seg", "target", True))
        tr_transforms.append(RenameTransform("seg2", "target2", True))
        ...
        tr_transforms.append(NumpyToTensor(["data", "target", "target2"], "float"))
        tr_transforms = Compose(tr_transforms)
        return tr_transforms
    ...
    def get_validation_transforms(
        ...
        val_transforms.append(RenameTransform("seg", "target", True))
        val_transforms.append(RenameTransform("seg2", "target2", True))
        ...
        val_transforms.append(NumpyToTensor(["data", "target", "target2"], "float"))
        val_transforms = Compose(val_transforms)
        return val_transforms
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.label_manager.has_ignore_label:
            if not self.label_manager.has_regions:
                ...
                # CAREFUL that you don't rely on target after this line!
                target[target == self.label_manager.ignore_label] = 0
                target2[target2 == self.label_manager.ignore_label] = 0
            else:
                mask = 1 - target[:, -1:]
                # CAREFUL that you don't rely on target after this line!
                target = target[:, :-1]
...
if __name__ == "__main__":
    ...
    # %%
    tmp = next(nnunet_trainer.dataloader_train)
